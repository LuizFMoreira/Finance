...
const PAGE_TITLES: Record<string, { title: string; subtitle: string }> = {
  "/dashboard": { title: "Visão Geral", subtitle: "Olá, Carlos! Aqui está seu resumo financeiro." },
...
export default function Topbar() {
  ...
  const page = PAGE_TITLES[location.pathname] ?? { title: "Pluma", subtitle: "" };
  ...
  return (
    <header className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-100 shadow-sm sticky top-0 z-30">
      ...
      <div className="flex items-center gap-3">
        {/* Search */}
        <div className="hidden md:flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 w-52 text-sm text-primary/40 cursor-pointer hover:border-primary/30 hover:bg-lavender-light transition-all">
        ...
        {/* User avatar */}
        <button className="flex items-center gap-2.5 px-2 py-1.5 rounded-xl hover:bg-lavender-light transition-all group">
          <div className="w-8 h-8 rounded-xl bg-primary flex items-center justify-center text-white text-sm font-700 shrink-0">
            C
          </div>
          <div className="hidden md:block text-left">
            <p className="text-xs font-600 text-primary leading-tight">Carlos Mendes</p>
            <p className="text-[10px] text-primary/40">Plano Pro</p>
          </div>
